...
class GCN(nn.Module):
    def __init__(self, g, num_layers, in_dim, num_hidden, num_classes):
        ...
        self.gcn_layers.append(GraphConv(in_dim, num_hidden))
        for _ in range(1, num_layers):
            self.gcn_layers.append(GraphConv(num_hidden, num_hidden))

        self.gcn_layers.append(GraphConv(num_hidden, num_classes))

    def forward(self, inputs, middle=False):
        h = inputs
        middle_feats = []
        for l in range(self.num_layers):
            h = self.gcn_layers[l](self.g.to(torch.device("cuda:0")), h)
            middle_feats.append(h)
            h = F.relu(h)
        logits = self.gcn_layers[-1](self.g.to(torch.device("cuda:0")), h)
        if middle:
            return logits, middle_feats
        return logits
